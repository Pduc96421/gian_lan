...
export default function MonitorPage() {
  ...
  useEffect(() => {
    if (selectedCaThi) {
      detectApi
        .getCamerasByCaThi(selectedCaThi)
        .then(setCameras)
        .catch(() => {});
  ...
  const fetchViolations = useCallback(async () => {
    ...
    try {
      ...
      console.error("Lỗi tải vi phạm:", e);
    }
  }, [selectedCameraCaThi]);
  ...
  const handleStartMonitor = async () => {
    ...
    connect(selectedCameraCaThi, selectedModel, modelPath, {
      onOpen: () => {
        ...
        const caThiObj = caThiList.find(
          (c) => String(c.id) === String(selectedCaThi),
        );
      ...
      <div className="page-hdr">
        <div>
          ...
          <p className="page-sub">
            Nhận dạng hành vi gian lận qua camera thời gian thực
          </p>
        </div>
      </div>
              ...
              >
                ...
                <div>
                  ...
                  <select
                    className="form-control"
                    value={selectedCaThi}
                    onChange={(e) => setSelectedCaThi(e.target.value)}
                    disabled={isMonitoring}
                  >
                    <option value="">-- Chọn ca thi --</option>
                    {caThiList.map((s) => (
                      <option key={s.id} value={s.id}>
                        {s.mon?.tenMon} - {s.phong?.tenPhong} ({s.thoiLuongPhut}{" "}
                        phút)
                      </option>
                      ...
                      //   key={s.id}
                      //   value={s.id}
                      //   disabled={s.trangThai !== 'CHUA_BAT_DAU'}
                      // >
                      //   {s.mon?.tenMon} - {s.phong?.tenPhong} ({s.thoiLuongPhut}{" "}
                      //   phút)
                      //   {s.trangThai === 'DANG_THI' ? ' [Đang diễn ra]' : s.trangThai === 'DA_KET_THUC' ? ' [Đã kết thúc]' : ''}
                      // </option>
                    ))}
                  </select>
                </div>
                <div>
                  ...
                  <select
                    className="form-control"
                    value={selectedCameraCaThi}
                    onChange={(e) => setSelectedCameraCaThi(e.target.value)}
                    disabled={isMonitoring || !selectedCaThi}
                  >
                    <option value="">-- Chọn camera --</option>
                    {cameras.map((c) => (
                      <option key={c.id} value={c.id}>
                        {c.camera?.tenCamera} - {c.camera?.viTri}
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div style={{ display: "flex", flexDirection: "column" }}>
                  {violations.map((v) => (
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        <div className="font-600 text-danger">
                          {v.hanhViGianLan}
                        </div>
                        <div className="text-sm mt-4">{v.chiTiet}</div>
                        <div className="text-xs text-muted mt-4">
                          {new Date(v.thoiDiemPhatHien).toLocaleString("vi-VN")}
                        </div>
                      </div>
                    </div>
                  ))}
