...
public interface KetQuaNhanDangRepository extends JpaRepository<KetQuaNhanDang, String> {
    ...
    @Query("SELECT COUNT(DISTINCT k.cameraCaThi.caThi.id) FROM KetQuaNhanDang k WHERE k.moHinhId = :moHinhId")
    long countDistinctCaThiByMoHinhId(String moHinhId);
    
    @Query("SELECT DISTINCT k.cameraCaThi.caThi FROM KetQuaNhanDang k WHERE k.moHinhId = :moHinhId")
    List<CaThi> findDistinctCaThiByMoHinhId(String moHinhId);

    @Query("SELECT k FROM KetQuaNhanDang k WHERE k.cameraCaThi.id = :cameraCaThiId AND k.moHinhId = :moHinhId ORDER BY k.thoiDiemPhatHien DESC")
