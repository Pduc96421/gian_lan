...
function MetricGrid({ model }) {
  return (
    <div className="metric-grid">
      {[['Accuracy', model.accuracy], ['Precision', model.precision], ['Recall', model.recall], ['F1-Score', model.f1Score]].map(([l, v]) => (
...
export default function ModelDetailPage() {
  ...
  const [progress,  setProgress]  = useState({ percent: 0, message: '', currentEpoch: null, totalEpochs: null })
  ...
  const loadSamples = useCallback(async (kw) => {
    ...
    setDsLoading(true)
    try { setSamples(await datasetApi.searchSamples(kw)) }
    catch { setSamples([]) }
  ...
  const handleStartTrain = async () => {
    try {
      ...
      setProgress({ percent: 0, message: 'Đang kết nối AI Service...', currentEpoch: null, totalEpochs: null })

      connect(id, [...selected], epochs, {
        onProgress: (evt) => setProgress({ percent: evt.percent, message: evt.message, currentEpoch: evt.currentEpoch ?? null, totalEpochs: evt.totalEpochs ?? null }),
        onCompleted: (evt) => {
          disconnect()
          setResult(evt)
  ...
  const handleCancelTrain = async () => {
    disconnect()
    try { await fetch(`http://localhost:8000/train/${id}`, { method: 'DELETE' }) } catch {}
    try { await modelApi.cancelTraining(id) } catch {}
    setModel(m => ({ ...m, trangThai: 'HOAN_THANH' }))
    setPhase(PHASE.DATASET)
  }
  ...
  if (loading) return <div className="loading-center"><div className="spinner" /> Đang tải...</div>
  ...
  return (
    <>
      ...
      <div className="page-hdr">
        ...
        <button className="btn btn-ghost" onClick={() => navigate('/models')}>← Quay lại</button>
      </div>
      ...
      <div className="card">
        ...
        <div className="card-body">
          ...
          <p className="text-sm text-muted">Đường dẫn: <code>{model.duongDanMoHinh || '—'}</code></p>
        </div>
      </div>
      ...
      <div className="card">
        <div className="card-hdr">
          ...
          <span className="text-sm text-muted">{trainedSamples.length} mẫu</span>
        </div>
        <div className="card-body">
          {trainedSamples.length === 0 ? (
            <p className="text-sm text-muted" style={{ margin: 0 }}>Không có thông tin tập dữ liệu đã huấn luyện.</p>
          ) : (
            <div className="dataset-grid" style={{ marginTop: 0 }}>
              {trainedSamples.map(s => (
                <div key={s.id} className="ds-card" style={{ cursor: 'default', opacity: 0.9 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div className="ds-name">{s.hanhViGianLan?.tenHanhVi || '—'}</div>
                    <span className={`badge ${s.usedType === 'TEST' ? 'badge-info' : 'badge-primary'}`} style={{ fontSize: '0.7rem', padding: '2px 6px' }}>
                      {s.usedType}
                    </span>
                  </div>
        ...
        <div className="card">
          <div className="card-hdr">
            ...
            <span className="text-sm text-muted">
              {selected.size ? `Đã chọn ${selected.size} mẫu` : 'Chưa chọn mẫu nào'}
            </span>
          </div>
          <div className="card-body">
            <div className="search-wrap">
              <span className="s-icon"></span>
              <input placeholder="Tìm theo tên hành vi gian lận..." value={dsKeyword} onChange={e => setDsKeyword(e.target.value)} />
            </div>

            {dsLoading ? (
              <div className="loading-center"><div className="spinner" /></div>
            ) : dsKeyword.trim() === '' ? (
              <div className="empty"><p>Nhập từ khoá để tìm kiếm tập dữ liệu...</p></div>
            ) : samples.length === 0 ? (
              <div className="empty"><p>Không tìm thấy mẫu phù hợp</p></div>
            ) : (
              <div className="dataset-grid">
                {samples.map(s => (
                  <div key={s.id} className={`ds-card${selected.has(s.id) ? ' selected' : ''}`} onClick={() => toggleSample(s.id)}>
                    <div className="ds-name">{s.hanhViGianLan?.tenHanhVi || '—'}</div>
                    <div className="ds-path">{s.duongDanDuLieu}</div>
                    <div className="ds-date">{s.ngayTao}</div>
                  </div>
        ...
        <div className="card">
          <div className="card-body">
            ...
            <div className="flex gap-8 mt-12">
              <button className="btn btn-danger" onClick={handleCancelTrain}>Huỷ huấn luyện</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="card">
          <div className="card-body">
            <div className="result-box">
              ...
              <div className="metric-grid">
                {[['Accuracy', result.accuracy], ['Precision', result.precision], ['Recall', result.recall], ['F1-Score', result.f1Score]].map(([l, v]) => (
                  <div key={l} className="metric-item">
                    <div className="metric-label">{l}</div>
                    <div className="metric-val">{fmt(v)}</div>
                  </div>
              ...
              <p className="text-sm text-muted">Đường dẫn mới: <code>{result.modelPath}</code></p>
            </div>
            <div className="flex gap-8 justify-end">
              <button className="btn btn-ghost" onClick={() => { setResult(null); setPhase(PHASE.DATASET) }}>Bỏ kết quả</button>
              <button className="btn btn-success" onClick={() => setShowModal(true)}>Xác nhận lưu mô hình</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              Bạn có chắc muốn cập nhật mô hình với kết quả huấn luyện mới?<br />
              Thao tác này sẽ ghi đè các chỉ số hiện tại.
            </div>
            <div className="modal-actions">
              <button className="btn btn-ghost" onClick={() => setShowModal(false)}>Hủy</button>
              <button className="btn btn-success" onClick={handleSave}>Xác nhận lưu</button>
            </div>
          </div>
        </div>
      )}
