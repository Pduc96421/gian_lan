...
const ModelSessionViolationsPage = () => {
  ...
  useEffect(() => {
    if (selectedCameraCaThiId) {
      ...
      detectApi
        .getViolationsByCameraAndModel(selectedCameraCaThiId, modelId)
        .then((res) => {
          setViolations(res || []);
    ...
    <div className="p-32 bg-light min-h-screen">
      ...
      <div className="flex items-center justify-between mb-32 bg-white p-24 rounded-16 shadow-sm border-0">
        <div className="flex items-center gap-20">
          ...
          <div>
            <h1 className="text-22 font-800 text-dark mb-4">
              Tra cứu Bằng chứng Vi phạm
            </h1>
            <div className="flex items-center gap-12 text-sm">
              <span className="text-muted">
                Mô hình:{" "}
                <strong className="text-primary">{model?.tenMoHinh}</strong>
              </span>
              <span className="text-muted border-start ps-12">
                Ca thi:{" "}
                <strong className="text-dark">
                  #{caThiId.substring(0, 8)}
                </strong>
              </span>
            </div>
          </div>
              ...
              {cameras.map((cam) => (
                ...
                >
                  <div className="overflow-hidden">
                    <div
                      className={`font-700 text-sm mb-2 ${selectedCameraCaThiId === cam.id ? "text-white" : "text-dark"}`}
                    >
                      {cam.camera?.tenCamera}
                    </div>
                    <div
                      className={`text-10 uppercase tracking-tighter ${selectedCameraCaThiId === cam.id ? "text-white-50" : "text-muted"}`}
                    >
                      ID: {cam.id.substring(0, 12)}...
                    </div>
                  </div>
                  {selectedCameraCaThiId === cam.id && (
          ...
          <div className="card border-0 shadow-sm rounded-16 bg-white min-h-600">
            {!selectedCameraCaThiId ? (
              <div className="card-body flex flex-col items-center justify-center p-80 opacity-60">
                ...
                <h4 className="text-18 font-700 text-dark mb-12">
                  Sẵn sàng tra cứu
                </h4>
                <p className="text-muted text-sm text-center max-w-300">
                  Hãy chọn một Camera từ danh sách bên trái để xem các bằng
                  chứng vi phạm mà mô hình đã phát hiện.
                </p>
              </div>
            ) : (
              <>
                  ...
                  {loadingViolations ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-24">
                      {violations.map((v, idx) => (
                        <div
                          key={idx}
                          className="card border-0 shadow-sm rounded-16 overflow-hidden bg-white hover-up transition-all"
                        >
                          <div className="relative group overflow-hidden">
                            <img
                              src={
                                v.hinhAnhMinhChungUrl
                                  ? v.hinhAnhMinhChungUrl.startsWith("http")
                                    ? v.hinhAnhMinhChungUrl
                                    : `http://localhost:8000${v.hinhAnhMinhChungUrl}`
                                  : ""
                              }
                              ...
                              onError={(e) => {
                                e.target.src =
                                  "https://via.placeholder.com/400x300?text=Loi+anh";
                              }}
                            />
                            <div className="absolute top-12 left-12">
                              <span
                                className="badge bg-danger shadow-sm px-10 py-6 font-700 uppercase"
                                style={{ fontSize: "10px" }}
                              >
                                {v.hanhViGianLan}
                              </span>
                          ...
                          <div className="card-body p-16">
                            <div className="flex justify-between items-center mb-12">
                              <span className="text-xs font-700 text-muted uppercase tracking-wider">
                                Thời điểm
                              </span>
                    ...
                    <div className="p-100 text-center opacity-40">
                      ...
                      <p className="text-muted text-sm">
                        Không tìm thấy bất kỳ hành vi gian lận nào từ góc máy
                        này.
                      </p>
                    </div>
                  )}
